variable "backend_settings" {
  type = list(object({
    name                    = string
    cookie_based_affinity   = string
    port                    = number
    protocol                = string
    request_timeout         = number
    pick_host_name_from_backend_address =  optional(string)
    probe_name              = optional(string)
  }))
  default = [
    {
      name                                = "Http-settings-80"
      cookie_based_affinity               = "Disabled"
      port                                = 80
      protocol                            = "Http"
      request_timeout                     = 20
      pick_host_name_from_backend_address = true
      probe_name                          = "http-80-health-probe"
    },
    {
      name                                = "favicon-http-settings-80"
      cookie_based_affinity               = "Disabled"
      port                                = 80
      protocol                            = "Http"
      request_timeout                     = 20
      pick_host_name_from_backend_address = true
      probe_name                          = "favicon-http-80-health-probe"
    },
    {
      name                                = "heartbeat-http-settings-80"
      cookie_based_affinity               = "Disabled"
      port                                = 80
      protocol                            = "Http"
      request_timeout                     = 20
      pick_host_name_from_backend_address = true
      probe_name                          = "heartbeat-http-80-health-probe"
    },
    {
      name                                = "Http-settings-82"
      cookie_based_affinity               = "Disabled"
      port                                = 82
      protocol                            = "Http"
      request_timeout                     = 20
      pick_host_name_from_backend_address = true
      probe_name                          = "http-82-health-probe"
    },
    {
      name                                = "Http-settings-9090"
      cookie_based_affinity               = "Disabled"
      port                                = 9090
      protocol                            = "Http"
      request_timeout                     = 20
      pick_host_name_from_backend_address = true
      probe_name                          = "healthz-http-9090-health-probe"
    },
    {
      name                                = "Http-settings-9190"
      cookie_based_affinity               = "Disabled"
      port                                = 9190
      protocol                            = "Http"
      request_timeout                     = 20
      pick_host_name_from_backend_address = true
      probe_name                          = "healthz-http-9190-health-probe"
    },
    {
      name                                = "Http-settings-9193"
      cookie_based_affinity               = "Disabled"
      port                                = 9193
      protocol                            = "Http"
      request_timeout                     = 20
      pick_host_name_from_backend_address = true
      probe_name                          = "healthz-http-9193-health-probe"
    },
    {
      name                                = "Http-settings-9194"
      cookie_based_affinity               = "Disabled"
      port                                = 9194
      protocol                            = "Http"
      request_timeout                     = 20
      pick_host_name_from_backend_address = true
      probe_name                          = "healthz-http-9194-health-probe"
    },
    {
      name                                = "Http-settings-9291"
      cookie_based_affinity               = "Disabled"
      port                                = 9291
      protocol                            = "Http"
      request_timeout                     = 20
      pick_host_name_from_backend_address = true
      probe_name                          = "swagger-index-http-9291-health-probe"
    },
    {
      name                                = "Http-settings-9292"
      cookie_based_affinity               = "Disabled"
      port                                = 9292
      protocol                            = "Http"
      request_timeout                     = 20
      pick_host_name_from_backend_address = true
      probe_name                          = "swagger-index-http-9292-health-probe"
    },
    {
      name                                = "Http-settings-9491"
      cookie_based_affinity               = "Disabled"
      port                                = 9491
      protocol                            = "Http"
      request_timeout                     = 20
      pick_host_name_from_backend_address = true
      probe_name                          = "healthz-http-9491-health-probe"
    },
    {
      name                                = "Http-settings-9590"
      cookie_based_affinity               = "Disabled"
      port                                = 9590
      protocol                            = "Http"
      request_timeout                     = 20
      pick_host_name_from_backend_address = true
      probe_name                          = "swagger-index-http-9590-health-probe"
    }
  ]
}
